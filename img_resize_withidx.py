...
from tools import inference_img, saveList, loadList
...
'''
Given Crop Size: (400, 400), Apply transform to the dataset
'''
cropped_height = 400
cropped_width = 400
transform = Compose([transforms.CenterCrop((cropped_height, cropped_width)),
                     transforms.ToTensor()])
img_usage_idx = loadList('img_idx_list.pkl')    # 用于训练的图片索引
