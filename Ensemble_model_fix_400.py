...
class FusionModel_400(nn.Module):
    '''
    输入: 三个模型的分割结果
    输出: 融合后的分割结果
    说明:
        ...
        这里总共学习的参数：150 x 400 x 400 x 3 = 90,000,000
    '''
    def __init__(self, class_num):
        super().__init__()
        # class_num: 150
        self.w1 = torch.nn.Parameter(torch.from_numpy(np.random.rand(class_num*400*400)).reshape(class_num, 400, 400))  
        self.w2 = torch.nn.Parameter(torch.from_numpy(np.random.rand(class_num*400*400)).reshape(class_num, 400, 400)) 
        self.w3 = torch.nn.Parameter(torch.from_numpy(np.random.rand(class_num*400*400)).reshape(class_num, 400, 400)) 
        
    def forward(self, input_seg1, input_seg2, input_seg3):
        # input_seg size: (150, 400, 400)
...
if train_flag:
    ...
    device_ids = [0, 1]
    if multi_GPU:
        model = nn.DataParallel(FusionModel_400(150), device_ids=device_ids)
    else:
        # 模型定义
        model = FusionModel_400(150)
    device = "cuda:0"
    ...
    torch.save(trained_model, model_save_path + "fusion_model_100_fix_400_Epoch_5.pth")
    ...
    np.save(model_save_path + 'data_dic_100_fix_400_Epoch_5.npy', data_dic)
